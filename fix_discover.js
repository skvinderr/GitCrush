...
const regex = /function TrendingDevs[\s\S]*?className="absolute -bottom-1 -right-1 w-4 h-4 bg-brand-green rounded-full border-2 border-black" \/>\s*<\/div>\s*\)\)}\s*<\/div>\s*<\/div>\s*<\/div>\s*\);\s*\}/;

const newStr = \unction TrendingDevs({ trending }) {
    ...
    return (
      <div className="absolute top-6 left-6 z-20 pointer-events-none flex flex-col gap-4 w-fit max-w-[200px] md:max-w-[240px]"> 
        <div className="flex flex-col gap-2">
          ...
          <div className="grid grid-cols-5 gap-2 pb-2 pointer-events-auto w-max max-w-full">
            {trending.map((dev) => (
              <div
                key={dev.id}
...
let newContent = content.replace(regex, newStr);
